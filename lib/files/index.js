...
(function() {
  var EventEmitter, File, fs, module, name, pathUtils, type, utils, _i, _len, _ref, _ref1,
  ...
  File = (function(_super) {
    ...
    function File(path, stats, compiler, registry) {
      ...
      fs.watch(this.sourcePath, function(event) {
        return fs.stat(_this.sourcePath, function(e, newStats) {
          if (!e) {
            return _this.onChange(newStats);
          }
        });
      });
    }
    ...
    File.prototype.isOutdated = function(cb) {
      var _this = this;
      if (this["private"]) {
        cb(new Error("Private files can't be outdated"));
      }
    ...
    File.prototype.isNewerThan = function(time) {
      var f, _i, _len, _ref;
      if (time < this.stats.mtime) {
        return true;
      }
      _ref = this.dependsOn;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        f = _ref[_i];
        if (f.isNewerThan(time)) {
          return true;
        }
      }
      return false;
