...
(function() {
  var createFolders, currentLevel, extension, first, forEach, fs, iterateFolder, logLevels, pathUtils, privateRegexp, utils,
    __slice = [].slice,
  ...
  currentLevel = process.env.NODE_ENV === 'test' && 10 || 1;

  exports.setLogLevel = function(level) {
    currentLevel = logLevels.indexOf(level);
    if (currentLevel === -1) {
      throw new Error("No log level named " + level);
    }
  };
  ...
  createFolders = exports.createFolders = function(path, cb) {
    ...
    return fs.exists(parent, function(exists) {
      if (exists) {
        return _create();
      } else {
    ...
    return fs.readdir(folder, function(err, files) {
      ...
      processFile = function(i) {
        ...
        next = function(err, paths) {
          if (err) {
            utils.log("debug", "Error iterating " + path + ": " + err);
          }
          if (paths) {
            results.push.apply(results, paths);
          }
  ...
  exports.isPrivate = function(path) {
    return privateRegexp.test(path);
  };

  exports.cleanPath = function(path) {
    return path.replace(privateRegexp, "$1");
  };
